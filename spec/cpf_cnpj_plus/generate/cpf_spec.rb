...
RSpec.describe CpfCnpjPlus::Generate::Cpf do
  describe '.generate' do
    ...
    it 'generates a CPF with correct check digits for a given base' do
      base = '123456789'
      cpf = described_class.generate(base)
      digits = described_class.calculate_digits(base)
      expect(cpf).to include(digits)
    end

    it 'generates a different CPF each time when base is not given' do
      allow(described_class).to receive(:rand).and_return(*0..8)
      cpf1 = described_class.generate
      allow(described_class).to receive(:rand).and_return(*1..9)
      cpf2 = described_class.generate
  ...
  describe '.calculate_digits' do
    ...
    it 'returns a string of two digits' do
      digits = described_class.calculate_digits('987654321')
